...
@asset(required_resource_keys={"postgres_credentials"},
       name="traffy_fondue_unpredicted_postgres_table",
       description="Create a PostgreSQL table for Traffy Fondue Unpredicted data",
       kinds={"postgres", "sql"},
       group_name="traffy_fondue",
       retry_policy=RetryPolicy(max_retries=3, delay=5),
       )
def traffy_fondue_unpredicted_postgres_table(context: OpExecutionContext) -> Dict[str, str]:
    # SQL to create table and indexes
    create_table_query = """
                         CREATE TABLE IF NOT EXISTS public.traffy_fondue_unpredicted
                         (
                         ...
                         ALTER TABLE public.traffy_fondue_unpredicted
                             OWNER TO postgres;

                         CREATE UNIQUE INDEX IF NOT EXISTS idx_traffy_fondue_unpredicted_ticket_id ON public.traffy_fondue_unpredicted (ticket_id);

                         CREATE INDEX IF NOT EXISTS idx_traffy_fondue_unpredicted_timestamp ON public.traffy_fondue_unpredicted (timestamp);
                         """
    ...
    try:
        ...
        return {
            ...
            "postgres_table": "traffy_fondue_unpredicted",
        }
